...
def compute_score_from_validated(profile: Mapping[str, Any]) -> float:
    """
    Compute score from a profile that is already normalized and validated.
    """

    score_component = float(profile["score"]) / 100.0

    # Experience contributes with diminishing returns to avoid overpowering score.
    experience_component = 1.0 - math.exp(-int(profile["experience"]) / 12.0)
